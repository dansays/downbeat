...
/** Rubric-confidence → emoji, shown on the calendar event title and the HTML rows. */
const CONFIDENCE_EMOJI: Record<NonNullable<SeenEvent["confidence"]>, string> = {
  strong: "🎯",
  good: "👍",
  tentative: "🤔",
};
const CONFIDENCE_LABEL: Record<NonNullable<SeenEvent["confidence"]>, string> = {
  strong: "Strong match",
  good: "Good match",
  tentative: "Worth a look",
};
...
export function buildIcs(events: SeenEvent[], opts: CalendarOptions): string {
  ...
  for (const ev of events) {
    ...
    const badge = ev.confidence ? `${CONFIDENCE_EMOJI[ev.confidence]} ` : "";
...
export function renderCalendarHtml(events: SeenEvent[], opts: CalendarOptions): string {
  ...
  const rows = events
    .map((ev) => {
      const location = opts.venueLocation?.(ev.venue) ?? ev.venue;
      const badge = ev.confidence
        ? `<span class="conf" title="${escapeHtml(CONFIDENCE_LABEL[ev.confidence])}">${CONFIDENCE_EMOJI[ev.confidence]}</span> `
        : "";
...
<body>
  <main class="wrap">
    ...
    <p class="legend">Match confidence: 🎯 strong &middot; 👍 good &middot; 🤔 worth a look</p>
${list}
