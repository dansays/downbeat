...
export function buildIcs(events: SeenEvent[], opts: CalendarOptions): string {
  ...
  for (const ev of events) {
    ...
    lines.push(`SUMMARY:${escapeText(`${ev.artist} — ${ev.venue}`)}`);
...
export function renderCalendarHtml(events: SeenEvent[], opts: CalendarOptions): string {
  ...
  const rows = events
    .map((ev) => {
      const location = opts.venueLocation?.(ev.venue) ?? ev.venue;
      const link = ev.ticketUrl
        ? ` &middot; <a href="${escapeHtml(ev.ticketUrl)}">tickets / info</a>`
        : "";
      ...
      return `      <li>
        ...
        <div class="what"><strong>${escapeHtml(ev.artist)}</strong> &mdash; ${escapeHtml(ev.venue)}${link}</div>
